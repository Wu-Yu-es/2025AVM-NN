...
# === 新增：支援 sample weight 的 DataLoader ===                                                            #避免大樣本組壓過小樣本組，逐樣本 loss 乘上權重後再平均，讓小樣本組合被「看得更重」
...
# === 修改：支援 sample weight 的訓練迴圈 ===
def train_once(X_tr, y_tr, X_va, y_va, input_dim: int, cfg, w_tr=None, w_va=None) -> Tuple[nn.Module, List[dict]]:
    ...
    crit = nn.L1Loss(reduction="none")  # 逐樣本 loss

    if w_tr is None:
        w_tr = np.ones((len(X_tr), 1), dtype=np.float32)
    if w_va is None:
        w_va = np.ones((len(X_va), 1), dtype=np.float32)
    ...
    for epoch in range(1, cfg.epochs + 1):
        ...
        history.append({"epoch": epoch, "train_mae": tr_mae, "valid_mae": va_mae})

        if epoch % 10 == 0 or epoch == 1:
            print(f"[Epoch {epoch:4d}] Train MAE = {tr_mae:.6f}, Valid MAE = {va_mae:.6f}")
...
def train_on_data(data: pd.DataFrame, cfg: TrainConfig):
    ...
    # （保持你原本的輸出存檔，若不需要可移除）
    df.to_csv("./Data_clean/Data_for_BPNN.csv", index=False)
...
if __name__ == "__main__":
    config = TrainConfig(
        ...
        id_cols=["CONTEXTID","EQID","CHAMBERID","RECIPEID"],
        # 補值策略
        impute_X_strategy="mean",      # 改成 "zero" 就是補 0
        impute_fit_scope="pre_only",   # 改成 "all" 會用整份資料的均值
        epochs=3000, patience=250, batch_size=2048, lr=1e-4, weight_decay=1e-4, num_blocks=20, dropout=0.2, hidden=250,
        # 裝置開關：False=ON(自動偵測CUDA); True=OFF(強制CPU)
        use_cpu_only=False,
        outlier_n = 6
    )
